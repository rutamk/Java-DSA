import java.util.*;
public class graph {
    static class Edge{ // making adjacency list which a way of creating a graph which has an array of arraylists(can also be made using hashmaps) the size of array is the number of vertexs and the arraylists store the Edge info such as source src destination dest weight wt (and directions if needed)
        int src; // source
    ...
    // 4. we iterate over the values of the arraylist we get from the findNeighbours function and add them ot the queue.
